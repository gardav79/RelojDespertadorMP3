...
bool DS3231::bisiesto(unsigned int ano)
{
  return ano%4==0&&(ano%100!=0||ano%400==0);
}

/*uint8_t DS3231::dias_mes(uint8_t mes, unsigned int ano)
{
  unsigned int plantilla_meses; // dos bytes (unsigned short)
  if(mes==2)
  {
    return 28+bisiesto(ano); // 28 + 1 si es bisiesto + 0 si no es bisiesto
  }
  else
  {
    plantilla_meses=0B0000101011010101<<(16-mes);
    return 30+((plantilla_meses>>15)==1); // 30 + 1 si es un mes largo + 0 si es un mes corto (menos febrero)
  }
}*/

char DS3231::dias_mes(char mes, char ano, char siglo)
{
  unsigned int plantilla_meses; // dos bytes (unsigned short)
  if(mes==2)
  {
    
    return 28+bisiesto((unsigned int)siglo*100+ano); // 28 + 1 si es bisiesto + 0 si no es bisiesto
  }
  else
  {
    plantilla_meses=0B0000101011010101<<(16-mes);
...
char DS3231::dias_mes(char mes, int ano)
{
  unsigned int plantilla_meses; // dos bytes (unsigned short)
  if(mes==2)
  {
    
    return 28+bisiesto((unsigned int)ano); // 28 + 1 si es bisiesto + 0 si no es bisiesto
  }
  else
  {
    plantilla_meses=0B0000101011010101<<(16-mes);
...
/*char DS3231::dia_semana(char dia, char mes, char ano, char siglo) // Cálculo del día de la semana por el algoritmo de la congruencia de Zeller
{
  ano-=(mes<=2);
  mes+=mes>2?-2:10;
  return (700+((26*mes-2)/10)+dia+ano+ano/4+siglo/4-2*siglo)%7; // «siglo» corresponde con las dos primeras cifras del año
}

char DS3231::dia_semana(char dia, char mes, char ano)
{
  return dia_semana(dia,mes,ano,SIGLO_XXI);
}*/


/*uint8_t DS3231::dia_semana(char dia, char mes, char ano)
{
  return dia_semana(dia,mes,ano,SIGLO_XXI);
}

uint8_t DS3231::dia_semana(char dia, char mes, char ano, char siglo) // Cálculo del día de la semana por el algoritmo de la congruencia de Zeller
{
  ano-=(mes<=2);
  mes+=mes>2?-2:10;
  return (700+((26*mes-2)/10)+dia+ano+ano/4+siglo/4-2*siglo)%7; // «siglo» corresponde con las dos primeras cifras del año
...
char *DS3231::getDOWStr(uint8_t format)
{
	...
	char *daysLong[]  = {"Lunes    ", 
		                 ...
		                 "Sabado   ", 
		                 "Domingo  "};
	char *daysShort[] = {"Lun", "Mar", "Mie", "Jue", "Vie", "Sab", "Dom"};
	Time t;
	t=getTime();
	if (format == FORMAT_SHORT)
...
char *DS3231::getMonthStr(uint8_t format)
{
	...
	char *monthLong[]  = {"Enero", 
		                  ...
		                  "Noviembre", 
		                  "Diciembre"};
	char *monthShort[] = {"Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"};
	Time t;
	t=getTime();
	if (format == FORMAT_SHORT)
